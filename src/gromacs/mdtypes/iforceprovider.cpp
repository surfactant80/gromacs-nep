...
bool ForceProviders::allForceProvidersSupportDeviceForces() const
{
    return !impl_->providers_.empty()
           && std::all_of(impl_->providers_.begin(),
                          impl_->providers_.end(),
                          [](const auto& provider)
                          { return provider.first->supportsDeviceForces(); });
}

DeviceBuffer<RVec> ForceProviders::deviceForceBuffer() const
{
    if (!allForceProvidersSupportDeviceForces() || impl_->providers_.size() != 1)
    {
        return {};
    }
    return impl_->providers_.front().first->deviceForceBuffer();
}

bool ForceProviders::deviceForcesValidForCurrentStep() const
{
    if (!allForceProvidersSupportDeviceForces() || impl_->providers_.size() != 1)
    {
        return false;
    }
    return impl_->providers_.front().first->deviceForcesValidForCurrentStep();
}
...
bool ForceProviders::haveDynamicCharges() const
{
    return std::any_of(impl_->providers_.begin(),
                       impl_->providers_.end(),
                       [](const auto& provider)
                       { return provider.first->supportsDynamicCharges(); });
}

bool ForceProviders::needCoulombPotential() const
{
    return std::any_of(impl_->providers_.begin(),
                       impl_->providers_.end(),
                       [](const auto& provider) { return provider.first->needsCoulombPotential(); });
}

bool ForceProviders::updateCharges(ArrayRef<const RVec>       x,
                                   DeviceBuffer<RVec>         deviceCoordinates,
                                   GpuEventSynchronizer*      deviceCoordinatesReady,
                                   ArrayRef<real>             chargeA,
                                   const interaction_const_t& ic,
                                   const matrix               box,
                                   const int                  homenr) const
{
    bool chargesChanged = false;
    for (auto& provider : impl_->providers_)
    {
        if (provider.first->supportsDynamicCharges())
        {
            chargesChanged = provider.first->updateCharges(x,
                                                           deviceCoordinates,
