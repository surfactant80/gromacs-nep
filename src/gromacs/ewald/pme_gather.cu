...
/*! \brief Sums the B-spline-interpolated reciprocal-space potential at one atom.
 *
 * The potential gather uses the B-spline values (not their derivatives) in
 * all three dimensions and requires no scaling: the same grid value is
 * gathered as in the force kernel, but with theta instead of dtheta.
 */
template<int order, int atomsPerWarp, bool wrapX, bool wrapY>
__device__ __forceinline__ void sumPotentialComponents(float* __restrict__ pot,
                                                       const int ithyMin,
                                                       const int ithyMax,
                                                       const int ixBase,
                                                       const int iz,
...
#pragma unroll
        for (int ithx = 0; (ithx < order); ithx++)
        {
            ...
            *pot += sm_theta[splineIndexX] * sm_theta[splineIndexY] * sm_theta[splineIndexZ]
                    * gridValue;
        }
    }
}

/*! \brief Reduces the partial potential contributions of one atom within its thread group. */
template<int atomDataSize, int blockSize>
__device__ __forceinline__ void reduce_atom_potential(float* sm_potential,
                                                      const int atomIndexLocal,
                                                      const int splineIndex,
                                                      const int lineIndex,
                                                      float      pot)
{
    if (gmx::isPowerOfTwo(atomDataSize))
    {
        const unsigned int activeMask = c_fullWarpMask;
        const int          width      = atomDataSize;
#pragma unroll
        ...
        {
            pot += __shfl_down_sync(activeMask, pot, delta, width);
        }
        if (splineIndex == 0)
        {
            sm_potential[atomIndexLocal] = pot;
        }
    }
    else
    {
        __shared__ float sm_potReduction[blockSize];
        sm_potReduction[lineIndex] = pot;
        __syncthreads();
        for (int redStride = atomDataSize / 2; redStride > 0; redStride >>= 1)
        {
            if (splineIndex < redStride)
            {
                sm_potReduction[lineIndex] +=
                        sm_potReduction[lineIndex + redStride];
            }
            __syncthreads();
        }
        if (splineIndex == 0)
        {
            sm_potential[atomIndexLocal] = sm_potReduction[lineIndex];
        }
    }
}
        ...
        void pme_gather_kernel(const PmeGpuKernelParams kernelParams)
{
    ...
    /* Optional per-atom reciprocal-space Coulomb potential. The potential is
     * gathered from the same real-space grid with the B-spline values in all
     * three dimensions and is written for the unperturbed state only. All
     * atoms are written, so no clearing is needed between steps. */
    float* __restrict__ gm_potential = kernelParams.atoms.d_potential;
    if (gm_potential != nullptr)
    {
        float pot = 0.0F;
        sumPotentialComponents<order, atomsPerWarp, wrapX, wrapY>(&pot,
                                                                  ithyMin,
                                                                  ithyMax,
                                                                  ixBase,
                                                                  iz,
                                                                  ...
                                                                  sm_theta,
                                                                  gm_gridA);
        __shared__ float sm_potential[atomsPerBlock];
        reduce_atom_potential<atomDataSize, blockSize>(
                sm_potential, atomIndexLocal, splineIndex, lineIndex, pot);
        __syncthreads();
        if (splineIndex == 0)
        {
            gm_potential[atomIndexGlobal] = sm_potential[atomIndexLocal];
        }
    }
