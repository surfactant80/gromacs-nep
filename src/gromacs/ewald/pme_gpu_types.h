...
struct PmeGpuAtomParams
{
    ...
    /*! \brief Global GPU memory array handle with the per-atom reciprocal-space
     * Coulomb potential output. Empty/null when the potential is not requested. */
    HIDE_FROM_OPENCL_COMPILER(DeviceBuffer<float>) d_potential;
};
