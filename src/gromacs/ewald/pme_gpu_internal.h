...
/*! \libinternal \brief
 * Enables or disables the per-atom reciprocal-space Coulomb potential output
 * of the PME gather. The device buffer is allocated lazily.
 *
 * \param[in] pmeGpu            The PME GPU structure.
 * \param[in] computePotential  Whether the potential should be computed.
 */
void pme_gpu_set_compute_potential(const PmeGpu* pmeGpu, bool computePotential);

/*! \libinternal \brief
 * Returns whether the per-atom reciprocal-space Coulomb potential is being
 * computed by the PME gather.
 */
bool pme_gpu_computes_potential(const PmeGpu* pmeGpu);

/*! \libinternal \brief
 * Returns the device buffer with the per-atom reciprocal-space Coulomb
 * potential, or an empty buffer when the potential is not computed.
 */
DeviceBuffer<float> pme_gpu_get_device_potential(const PmeGpu* pmeGpu);
