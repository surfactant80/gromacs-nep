...
/*! \brief Enable or disable the per-atom reciprocal-space Coulomb potential output.
 *
 * The potential is gathered from the PME grid with the B-spline values in all
 * three dimensions (no derivative) and written to a per-atom device buffer that
 * can be retrieved with pme_gpu_get_device_potential(). The device buffer is
 * allocated lazily and the potential is computed in the same gather kernel as
 * the forces, so pme_gpu_get_f_ready_synchronizer() also signals its readiness.
 *
 * \param[in] pme               The PME data structure.
 * \param[in] computePotential  Whether the potential should be computed.
 */
GPU_FUNC_QUALIFIER void pme_gpu_set_compute_potential(const gmx_pme_t* GPU_FUNC_ARGUMENT(pme),
                                                      bool GPU_FUNC_ARGUMENT(computePotential))
        GPU_FUNC_TERM;

/*! \brief Get pointer to the device buffer with the per-atom reciprocal-space Coulomb potential.
 * \param[in] pme            The PME data structure.
 * \returns                  Pointer to the per-atom potential data, or an empty buffer.
 */
GPU_FUNC_QUALIFIER DeviceBuffer<float> pme_gpu_get_device_potential(const gmx_pme_t* GPU_FUNC_ARGUMENT(pme))
        GPU_FUNC_TERM_WITH_RETURN(DeviceBuffer<float>{});

/*! \brief Upload the current atom charges to the PME GPU input coefficient buffer.
 *
 * This must be called whenever the charges change (e.g. every step with
 * dynamic charges). Only supported in single-precision builds.
 *
 * \param[in] pme             The PME data structure.
 * \param[in] h_coefficients  The host-side array of particle charges.
 */
GPU_FUNC_QUALIFIER void pme_gpu_update_input_coefficients(const gmx_pme_t* GPU_FUNC_ARGUMENT(pme),
                                                          const real* GPU_FUNC_ARGUMENT(h_coefficients))
        GPU_FUNC_TERM;
