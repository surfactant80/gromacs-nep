...
void pme_gpu_set_compute_potential(const gmx_pme_t* pme, const bool computePotential)
{
    if (!pme || !pme_gpu_active(pme))
    {
        return;
    }
    pme_gpu_set_compute_potential(pme->gpu, computePotential);
}

DeviceBuffer<float> pme_gpu_get_device_potential(const gmx_pme_t* pme)
{
    if (!pme || !pme_gpu_active(pme) || !pme_gpu_computes_potential(pme->gpu))
    {
        return DeviceBuffer<float>{};
    }
    return pme_gpu_get_device_potential(pme->gpu);
}

void pme_gpu_update_input_coefficients(const gmx_pme_t* pme, const real* h_coefficients)
{
    GMX_ASSERT(pme != nullptr && pme_gpu_active(pme), "PME GPU should be active.");
    GMX_ASSERT(h_coefficients != nullptr, "Null charge array passed to PME GPU.");
#if GMX_DOUBLE
    GMX_RELEASE_ASSERT(false, "Dynamic charge update requires a single-precision build.");
    GMX_UNUSED_VALUE(h_coefficients);
#else
    pme_gpu_realloc_and_copy_input_coefficients(
            pme->gpu, reinterpret_cast<const float*>(h_coefficients), 0);
#endif
}
