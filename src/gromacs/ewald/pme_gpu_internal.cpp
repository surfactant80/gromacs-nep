...
void pme_gpu_set_compute_potential(const PmeGpu* pmeGpu, const bool computePotential)
{
    auto* potential = &pmeGpu->kernelParams->atoms.d_potential;
    if (computePotential && *potential == nullptr && pmeGpu->kernelParams->atoms.nAtoms > 0)
    {
        allocateDeviceBuffer(potential,
                             pmeGpu->kernelParams->atoms.nAtoms,
                             pmeGpu->archSpecific->deviceContext_);
    }
    else if (!computePotential && *potential != nullptr)
    {
        freeDeviceBuffer(potential);
    }
}

bool pme_gpu_computes_potential(const PmeGpu* pmeGpu)
{
    return pmeGpu->kernelParams->atoms.d_potential != nullptr;
}

DeviceBuffer<float> pme_gpu_get_device_potential(const PmeGpu* pmeGpu)
{
    return pmeGpu->kernelParams->atoms.d_potential;
}
