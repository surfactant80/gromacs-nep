...
TEST_P(NNPotTopologyPreprocessorTest, FullSystemAvoidsGlobalExclusionGroup)
{
    const NNPotEmbedding embedding = GetParam();
    std::vector<int> nnpAtomIndices(12);
    std::iota(nnpAtomIndices.begin(), nnpAtomIndices.end(), 0);
    std::unique_ptr<gmx_mtop_t> mtop = makeMtopFromFile("4water", "");
    ...
    expectLogMessage("Neural network potential interface is active, topology was modified!");
    expectLogMessage("Number of embedded NNP atoms: 12\nNumber of regular atoms: 0\n");
    expectLogMessage("Full-system NNP detected: classical charges and non-bonded pair "
                     "parameters were disabled without constructing a global intermolecular "
                     "exclusion group.");
    expectLogMessage(
            "Number of settles removed: 4 \\(replaced by 8 "
            "InteractionFunction::ConnectBonds\\) \n");
    if (embedding == NNPotEmbedding::ElectrostaticModel)
    {
        expectLogMessage("Electrostatic embedding scheme is used.\n");
    ...
    EXPECT_TRUE(mtop->intermolecularExclusionGroup.empty());
    for (const auto& moleculeType : mtop->moltype)
    {
        for (int atomIndex = 0; atomIndex < moleculeType.atoms.nr; ++atomIndex)
        {
            EXPECT_EQ(moleculeType.atoms.atom[atomIndex].q, 0);
            EXPECT_EQ(moleculeType.atoms.atom[atomIndex].qB, 0);
        }
    }
    ...
    {
        EXPECT_EQ(mtop->ffparams.iparams[parameterIndex].lj.c6, 0);
        EXPECT_EQ(mtop->ffparams.iparams[parameterIndex].lj.c12, 0);
    }
}
