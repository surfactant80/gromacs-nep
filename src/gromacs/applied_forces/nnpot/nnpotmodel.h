...
public:
    ...
    virtual void evaluateModel(gmx_enerdata_t* /* enerData */,
                               ...
                               matrix* /* box */,
                               PbcType* /* pbcType */,
                               matrix* /* virial */ = nullptr) = 0;

    //! helper function to check if model outputs forces
    virtual bool outputsForces() const = 0;

    //! Evaluate a model using GROMACS device coordinates and a local-index map.
    virtual bool evaluateModelDevice(gmx_enerdata_t*,
                                     DeviceBuffer<RVec>,
    ...
    //! Device RVec force buffer from the most recent device evaluation.
