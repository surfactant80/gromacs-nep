...
public:
    ...
    void subscribeToPreProcessingNotifications(MDModulesNotifiers* notifiers) override
    {
        ...
        const auto adjustInputrecFunction = [this](t_inputrec* inputrec)
        { nnpotOptions_.adjustInputrec(inputrec); };
        notifiers->preProcessingNotifier_.subscribe(adjustInputrecFunction);
