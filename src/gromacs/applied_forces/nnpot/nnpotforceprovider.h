...
public:
    ...
    bool supportsDeviceForces() const override { return deviceForcePathEnabled_; }
    DeviceBuffer<RVec> deviceForceBuffer() const override;

    //! Whether the most recent calculateForces() filled the device force buffer.
    bool deviceForcesValidForCurrentStep() const override;

    //! Whether the qNEP model predicts dynamic charges handled by GROMACS.
    bool supportsDynamicCharges() const override;

    //! Whether the provider needs the GROMACS per-atom Coulomb potential.
    bool needsCoulombPotential() const override;

    //! Phase 1 of the qNEP-GROMACS electrostatics mode: predict charges.
...
private:
    //! Calculate adaptive molecule-wise delta-NEP corrections.
    void calculateAdaptiveForces(const ForceProviderInput& fInput, ForceProviderOutput* fOutput);

    //! Constant model energy offset for an atom list, in kJ/mol.
    double energyOffset(ArrayRef<const int> atomNumbers) const;
    ...
    //! Separate models keep adaptive A+B and B sizes from rebuilding the A model.
    std::shared_ptr<INNPotModel> adaptiveUnionModel_;
    std::shared_ptr<INNPotModel> adaptiveAModel_;
    std::shared_ptr<INNPotModel> adaptiveBModel_;

    //! Per-atomic-number constant energy offsets in kJ/mol/atom.
    std::unordered_map<int, double> energyOffsetByAtomicNumber_;
    ...
    //! True for the single-rank NEP GPU path (full system or link-free NEP/MM).
    bool deviceForcePathEnabled_ = false;

    //! True when the most recent calculateForces() call produced the device force buffer.
    bool deviceForcesProduced_ = false;

    //! Timings for the two qNEP-GROMACS phases (debug/profiling only).
    double lastUpdateChargesMs_ = 0;
    double lastCalculateForcesMs_ = 0;
};
