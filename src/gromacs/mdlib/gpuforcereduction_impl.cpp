...
void GpuForceReduction::Impl::registerSecondRvecForce(DeviceBuffer<RVec> forcePtr)
{
    GMX_ASSERT(forcePtr, "Second input force for reduction has no data");
    secondRvecForceToAdd_ = forcePtr;
};
...
void GpuForceReduction::Impl::execute()
{
    ...
    const bool addSecondRvecForce = static_cast<bool>(secondRvecForceToAdd_);
...
void GpuForceReduction::registerSecondRvecForce(DeviceBuffer<RVec> forcePtr)
{
    impl_->registerSecondRvecForce(forcePtr);
}
