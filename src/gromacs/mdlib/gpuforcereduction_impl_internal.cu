...
template<bool addRvecForce, bool addSecondRvecForce, bool accumulateForce>
static __global__ void reduceKernel(const float3* __restrict__ gm_nbnxmForce,
                                    const float3* __restrict__ rvecForceToAdd,
                                    const float3* __restrict__ secondRvecForceToAdd,
                                    float3*    gm_fTotal,
                                    const int* gm_cell,
                                    const int  numAtoms,
                                    ...
                                    const uint64_t forcesReadyNvshmemFlagsCounter)
{
    ...
    if (threadIndex < numAtoms)
    {
        ...
        if (addSecondRvecForce)
        {
            temp += secondRvecForceToAdd[threadIndex];
        }
                                ...
                                const uint64_t             forcesReadyNvshmemFlagsCounter)
{
    ...
    float3* d_secondRvecForceToAddPtr =
            addSecondRvecForce ? &(asFloat3(d_secondRvecForceToAdd)[atomStart]) : nullptr;
    ...
    auto kernelFn = addRvecForce
                            ? (addSecondRvecForce
                                       ? (accumulate ? reduceKernel<true, true, true>
                                                     : reduceKernel<true, true, false>)
                                       : (accumulate ? reduceKernel<true, false, true>
                                                     : reduceKernel<true, false, false>))
                            : (addSecondRvecForce
                                       ? (accumulate ? reduceKernel<false, true, true>
                                                     : reduceKernel<false, true, false>)
                                       : (accumulate ? reduceKernel<false, false, true>
                                                     : reduceKernel<false, false, false>));

    const auto kernelArgs = prepareGpuKernelArguments(kernelFn,
                                                      config,
                                                      &d_nbnxmForcePtr,
                                                      &d_rvecForceToAddPtr,
                                                      &d_secondRvecForceToAddPtr,
                                                      &d_baseForcePtr,
                                                      &d_cell,
                                                      &numAtoms,
