                                 ...
                                 bool                 didNeighborSearch)
{
    ...
    if (stepWork.computeForces)
    {
        ...
        if (domainWork.haveDeviceForceProviders && stepWork.useGpuFBufferOps)
        {
            GMX_ASSERT(stateGpu != nullptr,
                       "Device force providers require GROMACS GPU state buffers.");
            deviceCoordinates = stateGpu->getCoordinates();
            deviceCoordinatesReady = stateGpu->getCoordinatesReadyOnDeviceEvent(
        ...
        forceProviderInput.ic_            = fr->ic.get();
        forceProviderInput.computeEnergy  = stepWork.computeEnergy;
        forceProviderInput.computeVirial  = stepWork.computeVirial;
        if (stepWork.haveGpuPmeOnThisRank)
        {
            forceProviderInput.coulombPotential_ = pme_gpu_get_device_potential(fr->pmedata);
            forceProviderInput.coulombPotentialReady_ =
...
static int getExpectedLocalXReadyOnDeviceConsumptionCount(const SimulationWorkload& simulationWork,
                                                           ...
                                                           const bool haveDynamicCharges)
{
    int result = 0;
    if (domainWork.haveDeviceForceProviders && stepWork.computeForces
        && stepWork.useGpuFBufferOps)
    {
        // The device-capable force provider consumes the coordinate-ready
        // event before launching its model evaluation.
        result++;
    }
    if (haveDynamicCharges && stepWork.computeNonbondedForces)
    {
        // The dynamic-charge update hook consumes the coordinate-ready event
        // before predicting the new charges.
        result++;
    }
                                        ...
                                        const gmx_domdec_t*          dd)
{
    ...
    if (runScheduleWork.domainWork.haveDeviceForceProviders)
    {
        DeviceBuffer<RVec> deviceForce = forceProviders->deviceForceBuffer();
        GMX_RELEASE_ASSERT(deviceForce != nullptr,
                           "Device force provider did not expose a force buffer.");
        if (std::getenv("GMX_QNEP_DEBUG") != nullptr)
        {
            fprintf(stderr, "[qnep-debug] registering second force buffer %p\n",
                    static_cast<void*>(deviceForce));
        }
        gpuForceReduction->registerSecondRvecForce(deviceForce);
    }
              ...
              const DDBalanceRegionHandler& ddBalanceRegionHandler)
{
    ...
    if (stepWork.haveGpuPmeOnThisRank || stepWork.useGpuXBufferOps
        || pmeSendCoordinatesFromGpu || domainWork.haveDeviceForceProviders)
    {
        ...
        const int expectedLocalXReadyOnDeviceConsumptionCount =
                getExpectedLocalXReadyOnDeviceConsumptionCount(
                        ...
                        domainWork,
                        pmeSendCoordinatesFromGpu,
                        fr->forceProviders->haveDynamicCharges());
    ...
    /* Dynamic-charge force providers (e.g. qNEP): update the charges once the
     * coordinates are final, before the non-bonded and PME force computation. */
    if (stepWork.computeNonbondedForces && fr->forceProviders->haveDynamicCharges())
    {
        GMX_RELEASE_ASSERT(stateGpu != nullptr, "stateGpu should be available for dynamic charges");
        GMX_RELEASE_ASSERT(!simulationWork.havePpDomainDecomposition,
                           "Dynamic charge updates are not supported with domain decomposition.");
        // The dynamic-charge update is the only MD-loop operation that writes
        // to the atom charges after setup.
        auto* mdatomsWritable    = const_cast<t_mdatoms*>(mdatoms);
        const bool chargesChanged = fr->forceProviders->updateCharges(
                x.unpaddedArrayRef(),
                stateGpu->getCoordinates(),
                localXReadyOnDevice,
                mdatomsWritable->chargeA.subArray(0, mdatoms->homenr),
                *ic,
                box,
                mdatoms->homenr);
        if (chargesChanged)
        {
            /* Upload the new charges to the non-bonded and PME GPU data
             * structures before they are used to compute forces. */
            nbv->setAtomProperties(mdatoms->typeA,
                                   mdatomsWritable->chargeA,
                                   fr->atomInfo,
                                   mdatoms->typeB,
                                   mdatoms->chargeB);
            if (simulationWork.useGpuNonbonded)
            ...
            if (stepWork.haveGpuPmeOnThisRank)
            {
                pme_gpu_update_input_coefficients(fr->pmedata, mdatomsWritable->chargeA.data());
            }
        }
    }
    ...
    if (stepWork.haveGpuPmeOnThisRank)
    {
        pme_gpu_set_compute_potential(fr->pmedata, fr->forceProviders->needCoulombPotential());
    ...
    /* The alternating wait/reduce path consumes the non-bonded and PME
     * device force buffers, but not the force-provider device buffer. Add
     * the provider forces to the host force buffer here (single rank). */
    if (alternateGpuWait && domainWork.haveDeviceForceProviders
        && stepWork.computeNonbondedForces
        && fr->forceProviders->deviceForcesValidForCurrentStep())
    {
        DeviceBuffer<RVec> deviceForce = fr->forceProviders->deviceForceBuffer();
        if (deviceForce != nullptr)
        {
            GMX_RELEASE_ASSERT(!simulationWork.havePpDomainDecomposition,
                               "Device force providers with the alternating wait path "
                               "are only supported without domain decomposition.");
            const int numAtoms = nbv->getNumAtoms(AtomLocality::Local);
            std::vector<RVec> providerForces(numAtoms);
            cudaMemcpy(providerForces.data(),
                       deviceForce,
                       numAtoms * sizeof(RVec),
                       cudaMemcpyDeviceToHost);
            auto force = forceOutNonbonded->forceWithShiftForces().force();
            for (int i = 0; i < numAtoms; ++i)
            {
                force[i] += providerForces[i];
            }
        }
    }
    ...
    if (useOrEmulateGpuNb && !alternateGpuWait)
    {
        if (stepWork.useGpuFBufferOps)
        {
            ...
            if (stepWork.computeNonbondedForces)
            {
                if (std::getenv("GMX_QNEP_DEBUG") != nullptr)
                {
                    fprintf(stderr, "[qnep-debug] GpuForceReduction::execute\n");
                }
